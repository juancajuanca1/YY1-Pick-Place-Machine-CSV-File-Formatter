...
class ValidationEngine {
  ...
  static ProjectValidationState evaluate(PlacementProject? project) {
    ...
    final missingSlots = sideRecords
        .where((r) => r.feederSlot == null && !r.isFiducial)
        .length;
    final allHaveSlots = missingSlots == 0;
    if (!allHaveSlots) {
      errors.add(
        '$missingSlots component(s) are missing feeder slot assignments.',
      );
    }
    ...
    final unknownCount = project.records
        .where((r) => r.side == BoardSide.unknown)
        .length;
    if (unknownCount > 0) {
      warnings.add(
        '$unknownCount record(s) have an unrecognised side and are excluded.',
      );
    }

    // Warn about component warnings from normalization
    final recWarnings = project.records.expand((r) => r.warnings).toSet();
    if (recWarnings.isNotEmpty) {
      warnings.addAll(recWarnings.take(5));
      if (recWarnings.length > 5) {
        warnings.add(
          '… and ${recWarnings.length - 5} more normalisation warning(s).',
        );
      }
    }
